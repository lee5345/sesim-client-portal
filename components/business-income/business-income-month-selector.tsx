...
type BusinessIncomeMonthSelectorProps = {
  year: number;
  month: number;
  basePath: string;
  disabled?: boolean;
...
export function BusinessIncomeMonthSelector({
  year,
  month,
  basePath,
  disabled = false,
}: BusinessIncomeMonthSelectorProps) {
  ...
  function navigate(nextYear: number, nextMonth: number) {
    ...
    if (basePath.includes("/firm/companies/")) {
      params.set("tab", "business-income");
    }
  ...
  return (
    <>
      <div className="grid grid-cols-[auto_auto_1fr] grid-rows-[auto_auto] items-start gap-x-3 gap-y-1.5">
        <Label htmlFor="business-income-year" className="col-start-1 row-start-1">
          연도
        </Label>
        <Label htmlFor="business-income-month" className="col-start-2 row-start-1">
          월
        </Label>
        <span className="col-start-3 row-start-1 block h-5" aria-hidden="true" />

        <Input
          id="business-income-year"
          inputMode="numeric"
        ...
        <DialogContent className="sm:max-w-sm">
          ...
          <DialogFooter>
            <Button
              type="button"
              variant="outline"
              onClick={() => setYearErrorDialogOpen(false)}
            >
              확인
            </Button>
          </DialogFooter>
