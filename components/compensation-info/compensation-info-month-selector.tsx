...
type CompensationInfoMonthSelectorProps = {
  year: number;
  month: number;
  basePath: string;
  disabled?: boolean;
...
export function CompensationInfoMonthSelector({
  ...
  function navigate(nextYear: number, nextMonth: number) {
    ...
    if (basePath.includes("/firm/companies/")) {
      params.set("tab", "compensation-info");
    }
  ...
  return (
    <>
      <div className="grid grid-cols-[auto_auto_1fr] grid-rows-[auto_auto] items-start gap-x-3 gap-y-1.5">
        <Label htmlFor="compensation-info-year" className="col-start-1 row-start-1">
          연도
        </Label>
        <Label htmlFor="compensation-info-month" className="col-start-2 row-start-1">
          월
        </Label>
        <span className="col-start-3 row-start-1 block h-5" aria-hidden="true" />

        <Input
          id="compensation-info-year"
          inputMode="numeric"
